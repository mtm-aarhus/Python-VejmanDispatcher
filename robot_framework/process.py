...
def process(orchestrator_connection: OrchestratorConnection, queue_element: QueueElement | None = None) -> None:
    ...
    for case in cases:
        ...
        if row:
            ...
            if existing_folder != sharepoint_folder_path:
                orchestrator_connection.log_info(f"Renaming SharePoint folder for case {case_number}")
                ...
                # orchestrator_connection.log_info(f"Folder for case {case_number} is already correct. Updating LastUpdated.")
            ...
            orchestrator_connection.log_info(f"Creating new entry for case {case_number}")
    ...
    orchestrator_connection.log_info(f"Processed {len(cases)} cases.")
    ...
    orchestrator_connection.log_info(f"Deleted {len(old_folders)} old folders from SharePoint.")
...
def create_sharepoint_folder(folder_name, ctx: ClientContext, orchestrator_connection: OrchestratorConnection):
    orchestrator_connection.log_info(f"Creating SharePoint folder: {folder_name}")
...
def rename_sharepoint_folder(old_name, new_name, ctx: ClientContext, orchestrator_connection: OrchestratorConnection):
    orchestrator_connection.log_info(f"Renaming SharePoint folder: {old_name} → {new_name}")
